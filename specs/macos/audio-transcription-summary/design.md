...
`AppSettings` 構造体に `splitIntervalMinutes`（デフォルト30）を追加し、`settings.json` に保存する。起動時に `AppSettingsStore().load()` で復元し、変更時に即座に保存する。`isRealtimeEnabled` は既存の `AWSSettingsViewModel` で管理済み。
