...
CPU・メモリ使用状況をリアルタイム表示するステータスバー。Darwin フレームワークを使用してシステムリソース情報を取得し、メインウィンドウ下部に表示する。録音/録画中は経過時間を「録音中 MM:SS」形式で赤色表示する。

### 録音中の UI 制御

録音/録画中は以下の UI 要素を無効化し、誤操作を防止する:

- 設定画面を開くボタン（ツールバー）: `.disabled(isAnyCapturing)`
- 入力ソースの選択 Picker: `.disabled(isCapturing || isRecording)`
- ファイル分割時間の選択 Picker: `.disabled(isCapturing || isRecording)`
- FileDropZone: `isDisabled` パラメータで D&D とボタンを無効化、半透明表示
- 要約セクションの「ファイルから要約」ボタン: `.disabled(isAnyCapturing)`

### リアルタイム文字起こしの有効/無効切り替え

入力セクションのトグルスイッチでリアルタイム文字起こしの有効/無効を切り替える:

- **無効化時**: ストリーミング停止 → 音声バッファコールバック解除 → テキスト全クリア（文字起こし・翻訳両方）
- **有効化時（録音中）**: `startStreaming()` で新規ストリーミング開始 → 音声バッファコールバック再設定 → `streamOutputPath` が設定済みならファイルに追記

### 設定の永続化と即反映

`AppSettings` 構造体に `splitIntervalMinutes`（デフォルト30）を追加し、`settings.json` に保存する。起動時に `AppSettingsStore().load()` で復元し、変更時に即座に保存する。

設定画面の全項目は変更時に即座に保存・反映される:
- **macOS**: `AWSSettingsViewModel` で Combine の `Publishers.MergeMany` + `debounce(500ms)` で全 `@Published` プロパティの変更を監視し、`saveToFile()` を自動呼び出し
- **Windows**: 設定ダイアログの「保存」ボタンクリック時に `SettingsStore.Save()` を呼び出し、リアルタイムセクション表示・Bedrock モデル ComboBox を即更新

### 二重起動防止

- **macOS**: `NSRunningApplication.runningApplications(withBundleIdentifier:)` で同一バンドル ID のプロセス数を確認し、2つ以上なら `NSApplication.shared.terminate(nil)` で終了
- **Windows**: `System.Threading.Mutex` で名前付きミューテックスを作成し、`createdNew` が false なら `Environment.Exit(0)` で終了

### 起動時 AWS 接続テスト

起動時に `AWSSettingsViewModel.testConnection()` を自動実行し、失敗時は設定画面を自動表示する:
- **macOS**: `MainView.onAppear` で `testConnection()` を呼び出し、`connectionTestSuccess` が false なら `showSettings = true`
- **Windows**: `MainPage_Loaded` 末尾で `TestConnectionOnStartupAsync()` を呼び出し、例外発生時に `OnSettingsClick` で設定ダイアログを表示

### 設定画面のステータスバー

設定画面の下部に固定ステータスバーを表示し、AWS 接続状態を常時表示する:
- 接続済み（緑）/ エラーメッセージ（赤）/ 未検証（黄）/ 未設定（灰）
- 既存のインラインステータスラベルは廃止し、ステータスバーに統合
