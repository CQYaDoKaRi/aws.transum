...
録音中の状態管理:
- 録音中は設定ボタン、ファイル選択ボタン、ドラッグ＆ドロップ、ファイルから要約ボタン、要約ボタン、入力ソース選択、ファイル分割時間選択を無効化
- ステータスバーに「音声をキャプチャ中... mm:ss」形式で録音経過時間を表示（2秒ごと更新）
- 録音停止時: 入力セクションとリアルタイムセクションを折りたたみ、音声文字起こしと要約セクションを展開

リアルタイム言語再判別:
- 言語自動判別モード時に緑色更新アイコン（&#xE72C;、#16C60C）の再判別ボタンを表示
- クリックでリアルタイムストリーミングを再接続し言語判別をリセット
